...
export default function Home(){
  ...
  function eventsForDate(iso){
    ...
    return calendarEvents.filter(ev=>{
      ...
      if(ev.repeat_rule==="daily") return true;
      if(ev.repeat_rule==="none") return ev.event_date===iso;
      if(ev.repeat_rule==="weekly"){
        const base=new Date(ev.event_date+"T12:00:00");
        return base.getDay()===targetWeekday && iso>=ev.event_date;
      }
      return false;
    }).sort((a,b)=>(a.start_time||"00:00").localeCompare(b.start_time||"00:00"));
  }

  function todaysPersonalEvents(){
    return eventsForDate(todayISO());
  }

  function categoryIcon(category){
    return {
      workout:"🏋️",work:"💼",school:"🎓",meal:"🍱",
      weighin:"⚖️",recovery:"🧘",custom:"•"
    }[category]||"•";
  }

  async function addScheduleBlock(e){
    e.preventDefault();
    const fd=new FormData(e.currentTarget);
    const row={
      user_id:session.user.id,
      ...
      start_time:String(fd.get("start_time")||"08:00"),
      end_time:String(fd.get("end_time")||"09:00"),
      repeat_rule:String(fd.get("repeat_rule")||"weekly"),
      reminder_minutes:+fd.get("reminder_minutes")||0,
      notes:String(fd.get("notes")||"").trim(),
      color:String(fd.get("color")||"green"),
      enabled:true
    };
    if(!row.title){alert("Enter a schedule title.");return}
    const {error}=await sb.from("calendar_events").insert(row);
  ...
  return <main className="shell">
    ...
    <section className={"section "+(tab==="dashboard"?"active":"")}>
      ...
      <div className="nativeDashboardGrid">
        <div className="nativePanel todayWorkoutPanel">
          <div className="nativePanelHead">
            <div><small>YOUR DAY</small><h3>{new Date().toLocaleDateString(undefined,{weekday:"long"})}</h3></div>
            <button onClick={()=>goTo("schedule")}>Schedule</button>
          </div>
          {todaysPersonalEvents().length?
            <div className="personalTodayList">
              {todaysPersonalEvents().slice(0,4).map(ev=><div className="personalTodayEvent" key={ev.id}>
                <span>{categoryIcon(ev.category)}</span>
                <div><b>{ev.title}</b><small>{ev.start_time?.slice(0,5)}–{ev.end_time?.slice(0,5)}</small></div>
              </div>)}
            </div>
            :
            <div className="emptyScheduleState">
              <b>No schedule yet</b>
              <small>Add your own work, school, workout, and recovery blocks.</small>
              <button onClick={()=>goTo("schedule")}>Build my schedule</button>
            </div>
          }
        </div>
    ...
    <section className={"section "+(tab==="schedule"?"active":"")}>
      <NativeTitle title="My Schedule" subtitle="Your week is private to your account. Add work, school, workouts, meals, recovery, and anything else you need."/>

      <div className="grid g2">
        <form className="card" onSubmit={addScheduleBlock}>
          <h2>Add schedule block</h2>
          <div className="field"><label>Title</label><input name="title" placeholder="Work, Calculus, Push Day..." required/></div>
          <div className="calendarFormGrid">
            <div className="field"><label>Category</label><select name="category"><option value="work">Work</option><option value="school">School / Class</option><option value="workout">Workout</option><option value="meal">Meal prep</option><option value="recovery">Recovery</option><option value="weighin">Weigh-in</option><option value="custom">Other</option></select></div>
            <div className="field"><label>First date</label><input name="date" type="date" defaultValue={todayISO()} required/></div>
            <div className="field"><label>Starts</label><input name="start_time" type="time" defaultValue="09:00" required/></div>
            <div className="field"><label>Ends</label><input name="end_time" type="time" defaultValue="10:00" required/></div>
            <div className="field"><label>Repeat</label><select name="repeat_rule" defaultValue="weekly"><option value="none">One time</option><option value="daily">Every day</option><option value="weekly">Every week</option></select></div>
            <div className="field"><label>Reminder</label><select name="reminder_minutes" defaultValue="30"><option value="0">At start</option><option value="10">10 min before</option><option value="15">15 min before</option><option value="30">30 min before</option><option value="60">1 hour before</option><option value="1440">1 day before</option></select></div>
          ...
          <div className="field"><label>Notes</label><textarea name="notes" rows="2" placeholder="Optional..."/></div>
          <button className="btn">Add to my schedule</button>
        </form>

        <div className="card">
          <h2>Schedule tools</h2>
          <p className="muted">Your schedule is connected to your BenFit calendar and reminders. Changes here affect only your account.</p>
          <div className="scheduleTools">
            <button className="btn secondary" onClick={()=>goTo("calendar")}>Open calendar</button>
